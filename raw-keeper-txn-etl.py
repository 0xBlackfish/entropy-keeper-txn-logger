...
# if the dataframe is not empty, apply logic to add instruction types and reduce the dataframe to a set of six instructions, then write to google cloud storage
    ...
    df_reduced = df[df['instruction_type'].isin(['UpdateRootBank','CacheRootBanks','CachePerpMarkets','CachePrices','UpdateFunding','ConsumeEvents'])][['transaction.feePayer','instruction_type','transaction.signature']]
